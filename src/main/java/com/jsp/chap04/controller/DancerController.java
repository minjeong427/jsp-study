...
// 응답할 view를 결정합니다.
@WebServlet("*.do") // 확장자 패턴 -> 앞이 무엇이든 간에 .do로 끝나는 요청이라면 이 서블릿이 다 받겠다.
        ...
        try {
            switch (requestURI) {
                ...
                case "/regist.do":
                    ...
                    // 디스패처에게 목적지를 알려주면서, request와 response 객체를 가지고 이동하라는 명령을 내립니다.
                    // 실제 페이지가 목적지로 이동되면서,  request와 response 객체도 함께 전달됩니다.
                ...
                case "/list.do":
                    ...
                    sv = new ListService();
                    sv.execute(request, response);
                    rd = request.getRequestDispatcher("/WEB-INF/chapter03/dancer/list.jsp");
                    rd.forward(request, response);
                    break;
            }
        } catch (Exception e) {
